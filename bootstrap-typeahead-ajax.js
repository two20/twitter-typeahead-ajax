 ...
 * Modifications by Two Twenty Software Technologies Pvt Ltd - http://220tech.co.in
 */
...
!function($){
	...
	var Typeahead = function (element, options) {
		...
		this.onSelection = this.options.onSelect || null;
	...
	Typeahead.prototype = {
		...
		select: function () {
			...
			if(this.options.onSelection && (typeof this.options.onSelection == 'function'))
			{
				if(this.ajax.displayField)
				{
					var o = this.$menu.find('.active').attr('data-obj')
					this.options.onSelection(jQuery.parseJSON(unescape(o)));
				}
				else
				{
					this.options.onSelection(val);
				}
			}
			this.$element
		...
		hide: function () {
			...
			var dval = val.attr('data-value');
			var evl = this.$element.val();
			if(dval != evl)
			{
				if(this.options.onSelection && (typeof this.options.onSelection == 'function'))
				{
		...
		sorter: function (items) {
			...
			while (item = items.shift()) {
				...
				if (this.ajax && this.ajax.displayField) {
					it = item[this.ajax.displayField]
				}
		...
		render: function (items) {
			...
			items = $(items).map(function (i, item) {
				...
				{
					i=$(that.options.item).attr('data-obj', escape(JSON.stringify(item))).attr('data-value', item[that.ajax.displayField])
				}
				else
				{
